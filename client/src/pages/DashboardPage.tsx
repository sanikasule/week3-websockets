...
// import { useLiveMarketStore } from "../store";

// export const DashboardPage = memo(function DashboardPage() {
//   // const stocks        = useMarketStore((s) => s.stocks);
//   // const priceHistory  = useMarketStore((s) => s.priceHistory);
//   const stocks = useLiveMarketStore((s) => s.ticks);
//   const priceHistory = useLiveMarketStore((s) => s.tickHistory);
...
//   const filteredStocks = useMemo(() => {
//   const allTicks = Object.values(stocks); // 'stocks' is now 'ticks' from the store
  
//   // Map the raw ticks to the Stock format the table expects
//   return allTicks.map(tick => ({
//     symbol: tick.token, // Use token as symbol for now
//     name: `Token ${tick.token}`,
//     sector: "Market",
//     price: tick.ltp,
...
//       return filteredStocks.find(s => s.symbol === selectedSymbol) ?? null;
...
export const DashboardPage = memo(function DashboardPage() {
  // Pull from the correct store
  const ticks = useLiveMarketStore((s) => s.ticks);
  const tickHistory = useLiveMarketStore((s) => s.tickHistory);
  
  // UI selection state from marketStore
  const selectedSymbol = useMarketStore((s) => s.selectedSymbol);
  const setSelected = useMarketStore((s) => s.setSelected);
  ...
  // Map raw MarketTicks to Stock interface
  const mappedStocks = useMemo(() => {
    return Object.values(ticks).map((tick) => ({
      symbol: tick.token,
      name: `Instrument ${tick.token}`, // Note: Real names usually come from a master API
      sector: "Market",
      price: tick.ltp,
      change: tick.change ?? 0,
      changePercent: tick.changePercent ?? 0,
  ...
  const handleSort = useCallback((col: string) => {
    if (sortBy === col) setSortDir((d) => (d === "asc" ? "desc" : "asc"));
      ...
      setSortBy(col as keyof Stock);
      setSortDir("asc");
    }
  }, [sortBy]);

  // Derived selected stock
  const selectedStock = useMemo(() => 
    sortedStocks.find(s => s.symbol === selectedSymbol) ?? null
  , [sortedStocks, selectedSymbol]);

  const selectedHistory = selectedSymbol ? (tickHistory[selectedSymbol] ?? []) : [];

  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
      <div style={{ padding: "10px 20px", borderBottom: "1px solid var(--border)", background: "var(--bg-panel)", display: "flex", alignItems: "center", gap: "12px", flexShrink: 0 }}>
        <input
          placeholder="Search tokens..."
          value={searchText}
          onChange={(e) => setSearchText(e.target.value)}
          style={{ padding: "7px 12px", width: "300px", background: "var(--bg-elevated)", border: "1px solid var(--border)", borderRadius: "var(--radius)", color: "var(--text-primary)" }}
        />
        <span style={{ fontSize: "11px", color: "var(--text-muted)" }}>
          {sortedStocks.length} Active Ticks
        </span>
      </div>
